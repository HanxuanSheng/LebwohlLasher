...
#加上 0.1 作为一个最小扰动量，即使在极低温度（Ts ≈ 0）下，仍然允许微小的角度变化，使系统有机会逃离局部极小能量态，而不会完全锁死。
    ...
    #prng = random.RandomState(42)  # 使用固定种子可以确保结果可重复
    #aran = prng.normal(scale=scale, size=(nmax, nmax))
    #因为中值为0，所以不会产生波浪状或棋盘，比如说第一行平均比第二行少一个正态分布的中值。
    #在实际中，温度对于角度的影响，是如此随意的吗？正态分布，还是一个极为复杂的等式
    for ix in prange(nmax):#不需要额外的同步处理
        for iy in prange(nmax):#看是否里层要迭代行 更节省时间
            #ix = xran[i,j]
            #iy = yran[i,j]
            ang = aran[ix,iy]
